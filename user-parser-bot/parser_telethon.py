...
from geo2 import extract_and_geocode, configure, GeoResult


_geo_executor = ThreadPoolExecutor(max_workers=2)
...
def _geo_to_html(geo: GeoResult) -> str:
        """Format GeoResult as an HTML block for Telegram."""
        if not geo.parsed or geo.parsed.confidence < 0.3:
            return ""

        lines = ["\n<b>📍 Address detected:</b>"]
        lines.append(f"<code>{html.escape(geo.parsed.to_display_string())}</code>")

        if geo.geocoded:
            maps_url = f"https://www.google.com/maps?q={geo.latitude},{geo.longitude}"
            lines.append(f'<a href="{maps_url}">🗺 Open on map</a>  '
                        f'<code>{geo.latitude:.5f}, {geo.longitude:.5f}</code>')
        else:
            lines.append("<i>⚠️ Geocoding failed</i>")

        lines.append(f"<i>Method: {geo.method} | Confidence: {geo.parsed.confidence:.0%}</i>")
...
async def init_client() -> bool:
    """Initialize Telegram client with validation."""
    global client
    ...
    if not API_HASH:
        logger.error(
            ...
            "   3. Go to 'API Development Tools'\n"
            "   4. Create an Application\n"
            "   5. Copy API_HASH to .env file\n"
            "   Set: API_HASH=your_hash"
        )
        return False
...
async def on_new_message_handler(event: events.NewMessage.Event) -> None:
    ...
    if parsed.raw_text.strip():
        try:
            ...
            geo: GeoResult = await loop.run_in_executor(
                _geo_executor,
                extract_and_geocode,
                parsed.raw_text,
            )
            geo_html = _geo_to_html(geo)
            if geo_html:
                logger.info("    ├─ 📍 geo: %s  (%.0f%%)",
                            geo.parsed.to_display_string() if geo.parsed else "—",
                            (geo.parsed.confidence * 100) if geo.parsed else 0)
        except Exception as exc:
            logger.warning("Geo extraction failed: %s", exc)
    ...
    client, message, parsed.to_html() + geo_html, target,  # ← add geo_html
)
    ...
    target, parsed.to_html() + geo_html,  # ← add geo_html
    parse_mode="html", link_preview=False,
)
            except Exception as exc:
                logger.error("Send failed: %s", exc)
    else:
        print(parsed.to_html() + geo_html)
...
async def main() -> None:
    ...
    if not await init_client():
        logger.error("❌  Cannot proceed without valid API_ID and API_HASH")
        return
    ...
    client.on(events.NewMessage(chats=WATCH_CHATS or None))(on_new_message_handler)
    ...
    try:
        ...
        logger.error(
            ...
            "\n   Your API_ID/API_HASH combination is invalid or expired."
            "\n\n📝  HOW TO FIX:"
            "\n   1. Go to https://my.telegram.org/apps"
            "\n   2. Log in with your Telegram phone number"
            "\n   3. Click 'API Development Tools'"
            "\n   4. Delete the old application (if exists)"
            "\n   5. Create a NEW application:"
            "\n      - App title: Parser"
            "\n      - Short name: parser"
            "\n      - Click 'Create'"
            "\n   6. Copy the new api_id and api_hash"
            "\n   7. Update your .env file with NEW values:"
            "\n      API_ID=<new_id>"
            "\n      API_HASH=<new_hash>"
            "\n   8. Delete old session:"
            "\n      rm -f userbot_parser.session userbot_parser.session-journal"
