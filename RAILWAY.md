# Railway Deployment Guide

Полное руководство по деплою Laravel приложения на Railway.

## 🚀 Быстрый старт

### 1. Создайте новый проект на Railway

1. Войдите на [railway.app](https://railway.app)
2. Нажмите **New Project** → **Deploy from GitHub repo**
3. Выберите репозиторий `dnipro-map`
4. Railway автоматически обнаружит `Dockerfile` и начнет сборку

### 2. Настройте Environment Variables

Перейдите в **Variables** и добавьте следующие переменные:

#### 🔴 Обязательные переменные

```env
# Application
APP_NAME="Dnipro Map"
APP_ENV=production
APP_DEBUG=false
APP_URL=https://your-app.railway.app

# База данных (MySQL на Railway)
DB_CONNECTION=mysql
DB_HOST=${{MySQL.MYSQL_HOST}}
DB_PORT=${{MySQL.MYSQL_PORT}}
DB_DATABASE=${{MySQL.MYSQL_DATABASE}}
DB_USERNAME=${{MySQL.MYSQL_USER}}
DB_PASSWORD=${{MySQL.MYSQL_PASSWORD}}

# Логирование для Railway
LOG_CHANNEL=stderr
LOG_LEVEL=info

# Очередь (синхронная обработка)
QUEUE_CONNECTION=sync

# Telegram Bot
TELEGRAM_BOT_TOKEN=1234567890:ABCdefGHIjklMNOpqrsTUVwxyz
TELEGRAM_BOT_USERNAME=your_bot_username
WEB_APP_URL=https://your-app.railway.app/app

# Администраторы (через запятую)
ADMIN_TELEGRAM_ID=123456789,987654321

# Пароль админ-панели
ADMIN_PASSWORD=your_secure_password
```

#### 🟡 Опциональные переменные

```env
# Webhook для канала (рекомендуется)
CHANNEL_WEBHOOK_SECRET=random_secret_string_here

# Автоматическая настройка Telegram webhook
AUTO_SET_WEBHOOK=true

# Session & Cache
SESSION_DRIVER=database
CACHE_STORE=database
```

### 3. Добавьте MySQL на Railway

1. В проекте нажмите **New** → **Database** → **Add MySQL**
2. Railway автоматически создаст MySQL инстанс и переменные окружения
3. Используйте reference variables в настройках вашего сервиса (см. выше)

### 4. Настройте Volume для загруженных файлов

Railway автоматически создаст volume для `/data` (настроено в `railway.toml`).

Этот volume хранит:
- **Загруженные фото**: `/data/storage/photos/`

⚠️ **Важно**: Без volume загруженные фото потеряются при редеплое!

### 5. Получите URL приложения

После деплоя Railway присвоит URL вида: `https://your-app.up.railway.app`

Обновите переменные окружения:
```env
APP_URL=https://your-app.up.railway.app
WEB_APP_URL=https://your-app.up.railway.app/app
```

### 6. Настройте Telegram Webhook

#### Автоматически (если установлен `AUTO_SET_WEBHOOK=true`):
Webhook настроится автоматически при деплое.

#### Вручную через Railway Shell:
1. Откройте **Railway Dashboard** → ваш сервис
2. Перейдите в **Settings** → **Deploy** → **Shell**
3. Выполните команду:
```bash
php artisan telegram:set-webhook
```

#### Вручную через терминал:
```bash
railway run php artisan telegram:set-webhook
```

### 7. Проверьте деплой

- **Health check**: `https://your-app.railway.app/up` → должен вернуть `200 OK`
- **Админ-панель**: `https://your-app.railway.app/admin`
- **Web App**: `https://your-app.railway.app/app`
- **API**: `https://your-app.railway.app/api/points`

---

## 📋 Подробная документация

### Как получить Telegram Bot Token

1. Найдите [@BotFather](https://t.me/botfather) в Telegram
2. Отправьте `/newbot`
3. Следуйте инструкциям (выберите имя и username)
4. Скопируйте токен вида `1234567890:ABCdefGHIjklMNOpqrsTUVwxyz`
5. Добавьте в Railway: `TELEGRAM_BOT_TOKEN=<ваш_токен>`

### Как получить Telegram User ID

**Способ 1 - через бота:**
1. Найдите [@userinfobot](https://t.me/userinfobot) в Telegram
2. Отправьте `/start`
3. Скопируйте ваш ID (например, `123456789`)

**Способ 2 - через ваш бот:**
1. Временно включите дебаг: `APP_DEBUG=true`
2. Отправьте любое сообщение вашему боту
3. Проверьте логи Railway - найдете user ID в запросе
4. Не забудьте вернуть `APP_DEBUG=false`!

### Настройка Web App в Telegram

После деплоя настройте кнопку Web App в вашем боте:

1. Откройте [@BotFather](https://t.me/botfather)
2. Отправьте `/mybots` → выберите вашего бота
3. **Bot Settings** → **Menu Button** → **Configure menu button**
4. Введите URL: `https://your-app.railway.app/app`
5. Введите текст кнопки: `🗺️ Открыть карту`

### Структура базы данных

Приложение использует MySQL с автоматическими миграциями:

- `users` - пользователи Telegram
- `points` - опасные точки на карте
- `subscriptions` - подписки пользователей на уведомления
- `votes` - голоса за/против точек
- `comments` - комментарии к точкам
- `channel_messages` - сообщения из Telegram канала
- `jobs` - таблица очереди задач
- `cache` - кэш приложения

Миграции запускаются автоматически при каждом деплое.

### Загрузка фото

Фотографии сохраняются на persistent volume в `/data/storage/photos/`.

**API endpoint**: `POST /api/upload`
- Требует Telegram авторизацию
- Accepts: `multipart/form-data` с полем `photo`
- Returns: `{"url": "https://your-app.railway.app/storage/photos/filename.jpg"}`

**Важно**: Убедитесь что volume настроен, иначе фото потеряются при редеплое!

### Архитектура очередей

По умолчанию используется `sync` driver - задачи выполняются синхронно.

**Преимущества**:
- Простота - не требует отдельного worker процесса
- Нет дополнительных затрат на Railway

**Недостатки**:
- При большом количестве подписчиков (>100) создание точки может занять много времени
- Блокирует HTTP запрос

**Переход на асинхронную обработку**:
Если у вас много подписчиков, можно настроить отдельный worker:

1. Измените в Railway Variables:
   ```env
   QUEUE_CONNECTION=database
   ```

2. Добавьте новый Railway Service "Queue Worker":
   - Same GitHub repo
   - В **Start Command** укажите: `php artisan queue:work --tries=3`
   - Используйте те же Environment Variables
   - Монтируйте тот же Volume `/data`

### Мониторинг и логи

**Просмотр логов в реальном времени**:
```bash
railway logs
```

**Или через Dashboard**:
Railway Dashboard → ваш сервис → **Deployments** → последний деплой → **View Logs**

Все логи Laravel выводятся в `stderr` и доступны в Railway.

### Проблемы и решения

#### ❌ "Webhook not set" или бот не отвечает

**Решение**:
```bash
# Проверьте webhook
railway run php artisan tinker
>>> app(\App\Services\TelegramService::class)->getWebhookInfo()

# Установите webhook вручную
railway run php artisan telegram:set-webhook
```

#### ❌ "Database connection refused"

**Решение**:
Убедитесь что:
1. MySQL сервис добавлен в Railway проект
2. Reference variables (`${{MySQL.MYSQL_HOST}}` и т.д.) настроены правильно
3. Редеплойте приложение после настройки MySQL

#### ❌ "Photos disappear after redeploy"

**Решение**:
Убедитесь что volume монтируется в `/data` и проверьте логи:
```bash
railway logs | grep "Linking photos"
```

Должно быть сообщение: `Linking photos directory to persistent volume...`

#### ❌ "500 Error" на странице

**Решение**:
```bash
# Проверьте логи
railway logs

# Очистите кэш
railway run php artisan config:clear
railway run php artisan cache:clear
railway run php artisan view:clear
```

#### ❌ Медленная работа при создании точек

**Решение**: 
Если у вас >100 подписчиков, переключитесь на асинхронную очередь (см. раздел "Переход на асинхронную обработку").

### Railway CLI команды

**Установка Railway CLI**:
```bash
npm i -g @railway/cli
railway login
railway link
```

**Полезные команды**:
```bash
# Просмотр логов
railway logs

# Выполнить команду
railway run php artisan migrate

# Открыть shell
railway shell

# Просмотр переменных
railway variables

# Добавить переменную
railway variables set KEY=value
```

---

## 🔒 Безопасность

### Обязательно настройте:

1. **Смените пароль админки**: используйте сложный пароль для `ADMIN_PASSWORD`
2. **Ограничьте админов**: добавьте только доверенных пользователей в `ADMIN_TELEGRAM_ID`
3. **Защитите webhook канала**: установите `CHANNEL_WEBHOOK_SECRET`
4. **Отключите дебаг**: `APP_DEBUG=false` в production
5. **HTTPS обязателен**: Railway предоставляет SSL автоматически

### Переменные с секретами:

Никогда не коммитьте в Git:
- `TELEGRAM_BOT_TOKEN`
- `ADMIN_PASSWORD`
- `CHANNEL_WEBHOOK_SECRET`
- `APP_KEY` (генерируется автоматически)

---

## 📊 Масштабирование

### Horizontal Scaling

Railway поддерживает репликацию сервисов:
- Dashboard → Service → **Settings** → **Replicas**
- Установите количество реплик (требует платный план)

MySQL уже используется как shared БД, поэтому горизонтальное масштабирование работает из коробки.

---

## 📝 Checklist перед деплоем

- [ ] Получен Telegram Bot Token
- [ ] Получены Telegram User ID администраторов
- [ ] Создан проект на Railway
- [ ] Подключен GitHub репозиторий
- [ ] Настроены все обязательные Environment Variables
- [ ] Volume `/data` настроен и монтируется
- [ ] Приложение успешно задеплоено
- [ ] Health check `/up` возвращает 200 OK
- [ ] Telegram webhook настроен (`php artisan telegram:set-webhook`)
- [ ] Web App кнопка добавлена в бот (через @BotFather)
- [ ] Проверена отправка тестовой точки
- [ ] Проверена загрузка фото
- [ ] Проверена админ-панель
- [ ] `APP_DEBUG=false` в production
- [ ] Настроен сложный `ADMIN_PASSWORD`

---

## 🆘 Поддержка

**Документация**:
- [Railway Docs](https://docs.railway.app/)
- [Laravel Deployment Docs](https://laravel.com/docs/deployment)
- [Telegram Bot API](https://core.telegram.org/bots/api)

**Логи Railway**: 
```bash
railway logs --follow
```

**Laravel Tinker для отладки**:
```bash
railway run php artisan tinker
```

---

Готово! 🎉 Ваше приложение должно работать на Railway.
